...
def recovered(fa,gff):
    ...
    for line in open(gff,errors='ignore'):
        ...
        s,e=int(p[3])-1,int(p[4]); x=seq[s:e]; x=x.reverse_complement() if p[6]=='-' else x; x=norm(x)
...
for g in genes:
    ...
    for rid in sorted(ret):
        ...
        if len(s)>=3:
            prot=str(Seq(s).translate(table=code,to_stop=False)).replace('*','X'); aa.append(SeqRecord(Seq(prot),id=rid,description=''))
