...
prepare_smoke(){
  local wd="$ROOT/work/smoke_test"
  local generator="$wd/generate_smoke.py"

  mkdir -p "$wd"

  # Avoid `conda run ... python -` with a heredoc: stdin forwarding is not
  # reliable on some HPC systems. Execute a temporary script file instead.
  cat > "$generator" <<'PY'
...
random.seed(42)
seed = "".join(random.choice("ACGT") for _ in range(2000))
(wd / "seed.fasta").write_text(">smoke_seed\n" + seed + "\n")

def rc(s):
    return s.translate(str.maketrans("ACGT", "TGCA"))[::-1]

with gzip.open(wd / "smoke_R1.fastq.gz", "wt") as r1h, \
     gzip.open(wd / "smoke_R2.fastq.gz", "wt") as r2h:
    for i in range(400):
        start = (i * 3) % (len(seed) - 450)
        r1 = seed[start:start + 150]
        r2 = rc(seed[start + 300:start + 450])
        q = "I" * 150
        r1h.write(f"@smoke_{i}/1\n{r1}\n+\n{q}\n")
        r2h.write(f"@smoke_{i}/2\n{r2}\n+\n{q}\n")

(wd / "samples.tsv").write_text(
    "sample_id\tsource\tread1\tread2\taccession\n"
    "smoke\tlocal\t"
    + str((wd / "smoke_R1.fastq.gz").resolve())
    + "\t"
    + str((wd / "smoke_R2.fastq.gz").resolve())
    + "\t\n"
)
PY

  conda run -n pomito_core python "$generator" "$wd"
  rm -f "$generator"

  [[ -s "$wd/seed.fasta" ]] || {
    echo "[ERROR] smoke seed generation failed: $wd/seed.fasta" >&2
    return 1
  }
  [[ -s "$wd/smoke_R1.fastq.gz" && -s "$wd/smoke_R2.fastq.gz" ]] || {
    echo "[ERROR] smoke FASTQ generation failed in: $wd" >&2
    return 1
  }
  [[ -s "$wd/samples.tsv" ]] || {
    echo "[ERROR] smoke sample-sheet generation failed: $wd/samples.tsv" >&2
    return 1
  }
